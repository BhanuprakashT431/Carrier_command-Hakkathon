...
async function runE2E() {
  ...
  if (analysisId) {
    try {
      ...
      console.log(`✅ Comparison completed. Total metric rows: ${compRes.data.data.comparison.length}`);
    } catch (err) {
  ...
  try {
    ...
    console.log(`✅ Readiness retrieved. Score: ${readinessRes.data.data.careerReadiness}`);
